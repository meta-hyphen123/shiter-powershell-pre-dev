...
if __name__ == "__main__":
    ...
    # 从终端读取用户输入的URL等信息
    url = input("Enter the target URL (e.g., \"http://www.target.com/page.php?id=1\"): ")
    data = input("Enter the POST data (e.g., \"query=test\"): ")
    cookie = input("Enter the HTTP Cookie header value: ")
    ua = input("Enter the HTTP User-Agent header value (press Enter to use default): ")
    referer = input("Enter the HTTP Referer header value: ")
    proxy = input("Enter the HTTP proxy address (e.g., \"http://127.0.0.1:8080\"): ")
